...
from typing import Any, AsyncIterator, Callable, ClassVar, Coroutine, Generator, List, Optional, Protocol, Tuple
...
# NVTX-range color palettes live as ``ClassVar`` on the handle
# classes that own them (:class:`Batch` / :class:`Concern`); see
# the docstrings on those classes for the OKLCh recipe and how to
# re-generate them.
...
class _Handle(Protocol):
    ...
    All three carry the same minimal shape: a ``coro`` they wrap, a
    ``done`` flag the Driver flips when that coro terminates, and a
    pair of label attributes (``nvtx_label`` / ``hang_label``) the
    Driver consults for the on-stack NVTX range and the watchdog's
    heartbeat. ``nvtx_label`` packs both the range message and its
    palette color into one ``Optional[Tuple[message, color]]`` --
    they are always read together, so a single attribute keeps the
    push site one assignment and the suppression check (``ann is
    None``) trivially clear. Each handle picks the form most
    useful for its role:

    +----------------------+--------------------------------------+----------------------+
    | handle               | ``nvtx_label``                       | ``hang_label``       |
    +======================+======================================+======================+
    | :class:`_RootHandle` | ``None`` (suppress NVTX)             | Scheduler <qualname> |
    +----------------------+--------------------------------------+----------------------+
    | :class:`Batch`       | ``("B<idx>",     palette[idx])``     | Batch <idx>          |
    +----------------------+--------------------------------------+----------------------+
    | :class:`Concern`     | ``("C:<qualname>", palette[hash])``  | Concern <qualname>   |
    +----------------------+--------------------------------------+----------------------+

    The root suppresses NVTX entirely (``nvtx_label is None``) -- a
    scheduler-rooted range would cover the whole run and add no
    per-iter signal. The Driver checks for ``None`` inside
    :meth:`Driver._stack_push` and skips the ``nvtx.push_range`` /
    ``pop_range`` pair for that handle.

    Resolution split:

    - ``hang_label`` is eager (set in ``__post_init__`` /
      ``__init__``). The watchdog reads it once per ``_drive`` loop
      iteration regardless of whether NVTX is enabled, so caching
      avoids re-formatting on the hot path.
    - ``nvtx_label`` is lazy (:func:`functools.cached_property` on
      :class:`Batch` / :class:`Concern`; constant ``None`` on
      :class:`_RootHandle`). It is only read inside
      ``if _NVTX_ENABLED:`` branches in :meth:`Driver._stack_push`
      / :meth:`Driver._stack_pop`, so when ``TLLM_COROUTINE_NVTX``
      is off the message + palette work NEVER runs -- making the
      on-demand resolution a true no-op for production workloads.
      When NVTX is on, the first read computes & caches the
      ``(message, color)`` tuple; the matching pop's re-read is
      free.

    This Protocol lets the Driver's internals annotate stack entries
    and advance-request payloads precisely without forward-referencing
    every concrete handle class.
    ...
    nvtx_label: Optional[Tuple[str, str]]
    hang_label: str
...
def _coro_qualname(coro: object) -> str:
    """``coro.__qualname__`` with a ``repr`` fallback for non-coroutine handles."""
...
class Batch:
    ...
    ``idx`` is the iteration counter of the scheduler that produced
    this batch; the Driver uses it to disambiguate concurrently-alive
    batches in NVTX ranges (``B<idx>: <name>``) and watchdog hang
    messages (``Batch <idx>: <qualname>``). Defaults to ``0`` so that
    test harnesses that build a single batch in isolation don't have
    to thread a counter; production schedulers always pass the live
    iter index.
    ...
    _NVTX_PALETTE: ClassVar[list[str]] = [
        '#f3a3bb', '#a2cd8b', '#a4bbff', '#f4a988',
        '#6dd3c0', '#d6aaea', '#d7bb70', '#70ccee'
    ]
    ...
    def __post_init__(self) -> None:
        # ``hang_label`` is read by the watchdog on every iter
        # regardless of NVTX, so resolve it eagerly. ``nvtx_label``
        # is ``cached_property`` below so it costs nothing when
        # ``TLLM_COROUTINE_NVTX`` is off (the gate in
        # :meth:`Driver._stack_push` never touches it) and one
        # f-string + one palette lookup on the first push when on.
        self.hang_label: str = f"Batch {self.idx}"

    @functools.cached_property
    def nvtx_label(self) -> Optional[Tuple[str, str]]:
        return (
            str(self.idx),
            Batch._NVTX_PALETTE[self.idx % len(Batch._NVTX_PALETTE)],
        )
...
class Concern:
    ...
    _NVTX_PALETTE: ClassVar[list[str]] = [
        '#f7a3ac', '#77d3b4', '#e1a7df', '#afc981',
        '#b3b6fd', '#e0b771', '#7dc8f6', '#f7a692',
        '#66d2ce', '#efa3c7', '#92d098', '#ccaef2',
        '#cac072', '#98bffe', '#efad7d', '#69cee6'
    ]

    _NVTX_REGISTRY: ClassVar[dict[str, str]] = {}

    coro: _CoroutineLike
    done: bool = False

    def __post_init__(self) -> None:
        # See :class:`Batch.__post_init__`: ``hang_label`` is eager
        # (watchdog hot path), ``nvtx_label`` is lazy below.
        #
        # ``__qualname__`` already encodes ``ClassName.method`` for
        # bound async-generator methods (the canonical concern
        # shape: ``ResourceConcern.handle_batch``) and degrades to
        # the bare function name for free functions, so the same
        # expression covers both.
        self.hang_label: str = f"Concern {_coro_qualname(self.coro)}"

    @classmethod
    def _get_color(cls, name: str):
        color = cls._NVTX_REGISTRY.get(name)
        if not color:
            size = len(cls._NVTX_REGISTRY)
            color = cls._NVTX_PALETTE[size % len(cls._NVTX_PALETTE)]
            cls._NVTX_REGISTRY[name] = color
        return color

    @functools.cached_property
    def nvtx_label(self) -> Optional[Tuple[str, str]]:
        qn = _coro_qualname(self.coro)
        return qn, self._get_color(qn)
...
class _RootHandle:
    ...
    NVTX is intentionally suppressed (``nvtx_label`` is ``None``)
    because a scheduler-rooted range covers the entire run -- it
    spans every iter and conveys no per-iter information on the
    timeline. Per-iter granularity comes from the :class:`Batch`
    and :class:`Concern` ranges nested inside.
    """

    __slots__ = ("coro", "done", "hang_label")

    # Class-level constant: ``None`` opts out of the NVTX range
    # entirely (per the class docstring). Defining it on the class
    # -- rather than as a ``__slots__`` slot written in
    # ``__init__`` -- keeps the suppression read free of any
    # per-instance work and lets :meth:`Driver._stack_push` /
    # :meth:`Driver._stack_pop` hit a single fast attribute
    # lookup.
    nvtx_label: Optional[Tuple[str, str]] = None

    def __init__(self, coro: _CoroutineLike) -> None:
        self.coro: _CoroutineLike = coro
        self.done: bool = False
        # ``hang_label`` is read by the watchdog on every iter
        # while the root sits at the bottom of the stack, so cache
        # it eagerly here.
        self.hang_label: str = f"Scheduler {_coro_qualname(self.coro)}"
...
class Driver:
    ...
    def _stack_push(self, handle: _Handle,
        ...
        Annotation comes from ``handle.nvtx_label`` -- a single
        ``Optional[Tuple[message, color]]`` lazily resolved on
        first read. When it is ``None`` (e.g., :class:`_RootHandle`
        -- a scheduler-rooted range covers the entire run and adds
        no per-iter signal) the push is silent and the matching
        :meth:`_stack_pop` will also skip the ``nvtx.pop_range``
        call. Color is the only knob that lets nsys visually
        distinguish ranges within a domain, so distinct iters
        (:class:`Batch`, indexed by ``idx``) and concern types
        (:class:`Concern`, indexed by ``__qualname__``) get
        different hues from pre-generated OKLCh palettes.
        """
        self._stack.append((handle, target))
        if _NVTX_ENABLED:
            ann = handle.nvtx_label
            if ann is None:
                return
            message, color = ann
            ...
            # target is ``None``) is the case the suppression above
            # already handled, so the suffix only ever appends here.
            if target is not None:
                message = f"{message}->{target.name}"
            nvtx.push_range(
                message, color=color, domain=_NVTX_DOMAIN,
            )
    ...
    def _stack_pop(self) -> Tuple[_Handle, Optional[IntEnum]]:
        ...
        The suppression decision re-reads ``handle.nvtx_label``:
        ``None`` on :class:`_RootHandle` (a class-level attribute,
        free) to opt out, the ``(message, color)`` tuple cached
        during the matching push on :class:`Batch` /
        :class:`Concern` (a ``cached_property`` hit, free). No
        re-formatting on the hot path either way.
        """
        entry = self._stack.pop()
        if _NVTX_ENABLED:
            handle, _ = entry
            if handle.nvtx_label is not None:
    ...
    def _drive(self) -> None:
        ...
        while self._stack:
            ...
            # the scheduler root).
            #
            # The handle exposes the structured label via
            # ``hang_label`` (``Scheduler <qualname>`` for the root,
            # ``Batch <idx>: <qualname>`` for batches, ``Concern:
            # <qualname>`` for concerns) so a fired hang report
            # immediately tells the reader which layer is wedged.
            # Strings only -- the watchdog never holds a coroutine
            # reference, so a finished coroutine GCs promptly even
            # if the watchdog thread is still alive.
            if self._watchdog is not None:
