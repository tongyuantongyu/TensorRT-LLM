...
from ..virtual_memory import BackedMode, maybe_scope
...
def create_py_executor(
    ...
    with (mem_monitor.observe_creation_stage(
            _ExecutorCreationStage.MODEL_ENGINE_MAIN),
          maybe_scope(enable_sleep, 'model', BackedMode.PINNED) as pool):
    ...
    if has_draft_model_engine:
        with (mem_monitor.observe_creation_stage(
                _ExecutorCreationStage.MODEL_ENGINE_DRAFT),
              maybe_scope(enable_sleep, 'draft_model', BackedMode.PINNED) as
              pool):
    ...
    if spec_resource_manager is not None:
        with maybe_scope(enable_sleep, 'spec', BackedMode.PINNED) as pool:
    ...
    with (mem_monitor.observe_creation_stage(_ExecutorCreationStage.DRAFTER),
          maybe_scope(enable_sleep, 'drafter', BackedMode.PINNED) as pool):
    ...
    with (mem_monitor.observe_creation_stage(
        ...
        py_executor = create_py_executor_instance(
            ...
            vm_pools if not estimating_kv_cache else None)
    ...
    if estimating_kv_cache:
        ...
        with (mem_monitor.observe_creation_stage(
                _ExecutorCreationStage.KV_CACHE),
              maybe_scope(enable_sleep, 'kv_cache', BackedMode.NONE) as pool):
        ...
        with (mem_monitor.observe_creation_stage(
                _ExecutorCreationStage.EXTRA_RESOURCES),
              maybe_scope(enable_sleep, 'extra', BackedMode.PINNED) as pool):
            del vm_pools['extra']
            vm_pools['extra'] = pool
            py_executor = create_py_executor_instance(
                ...
                garbage_collection_gen0_threshold, vm_pools)
