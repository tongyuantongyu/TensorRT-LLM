...
from ..virtual_memory import RestoreMode, scope as virtual_memory_scope
...
class _ExecutorMemoryMonitor:
    ...
    memory_type_friendly_names = {
        ...
        ExecutorMemoryType.INIT_KV_CACHE: "Initial KV Cache (temporary for KV cache size estimation)",
        ExecutorMemoryType.INIT_EXTRA_RESOURCES: "Additional executor resources (temporary for KV cache size estimation)",
        ...
        ExecutorMemoryType.MODEL_ENGINE_DRAFT: "Draft model for speculative decoding",
    }
...
def create_py_executor(
    ...
    @contextmanager
    def allocation_scope(current_stage: ExecutorMemoryType, restore_mode: RestoreMode):
    ...
    with allocation_scope(ExecutorMemoryType.MODEL_ENGINE_MAIN, RestoreMode.PINNED):
    ...
    if has_draft_model_engine:
        with allocation_scope(ExecutorMemoryType.MODEL_ENGINE_DRAFT, RestoreMode.PINNED):
    ...
    if guided_decoding_config is not None:
        with allocation_scope(ExecutorMemoryType.GUIDED_DECODER, RestoreMode.PINNED):
    ...
    if model_engine.model.model_config.is_generation:
        ...
        with allocation_scope(
                ExecutorMemoryType.INIT_KV_CACHE
                if estimating_kv_cache else ExecutorMemoryType.KV_CACHE, RestoreMode.NONE):
    ...
    with allocation_scope(ExecutorMemoryType.SPEC_RESOURCES, RestoreMode.PINNED):
    ...
    if estimating_kv_cache:
        ...
        with allocation_scope(ExecutorMemoryType.MODEL_EXTRA, RestoreMode.PINNED):
        ...
        with allocation_scope(ExecutorMemoryType.EXTRA_RESOURCES, RestoreMode.PINNED):
            py_executor = create_py_executor_instance(
                dist=dist,
                resources=resources,
                mapping=mapping,
