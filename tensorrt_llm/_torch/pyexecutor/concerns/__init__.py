...
::
    ...
    class XxxConcern:
        ...
        async def handle_batch(self, ctx: Context):
            # multi-phase coroutine. ``ctx`` exposes svc / conf /
            # state / port -- NEVER ``ctx.crn`` (concerns are
            # mechanically denied peer access).
            r0, _ = await enter_phase(BatchPhase.SCHEDULE_0)
            ...
            r2, _ = await enter_phase(BatchPhase.FORWARD_2)
            ...

        # OR a single-phase plain method:
...
inside ``async with batch_phase(P): ...`` blocks. Single-phase
methods are called directly. See :mod:`py_executor_coro` for the
plain-loop ``batch_body``.
"""
