...
**All concerns are coroutines.** Even a single-phase concern is shaped as `async def handle_batch(self, ctx)` with one `await enter_phase(P)` rendezvous, wrapped in `Concern(...)`, and driven from the BATCH body via `await resume(handle)` inside the matching `async with batch_phase(P): ...` block.

The two earlier reasons to prefer a coroutine — top-to-bottom lifecycle and locality of intra-concern state — still apply. The decisive third reason makes the rule uniform:

- **Automatic NVTX annotation.** The runtime wraps every handle on the Driver stack in an NVTX range (gated by `TLLM_COROUTINE_NVTX=1`). Each `Concern` handle automatically opens a `C<ClassName>.handle_batch` range at push time and closes it at pop. A plain method called from the BATCH body bypasses the stack and is therefore invisible on the timeline — there is no place for the runtime to clip a range. Wrapping the method in a coroutine is the cheapest way to get uniform tracing without per-concern `nvtx.push_range` / `pop_range` plumbing.

The boilerplate cost is one line at the top:
...
A plain method is still the right shape for the cross-concern peer hook (see the `record_acceptance` example further down), because that hook is invoked synchronously by another concern at a specific point inside its own coroutine -- it isn't a phase participant.

### Why a coroutine even without state-threading

Concerns like `DisaggConcern` or `ResourceConcern` could in principle expose two or three methods (e.g., `Resource.prepare(...)` at RESOURCE_PREP_1, `Resource.update(...)` at RESPOND_8), called from the BATCH body at the right phases. We don’t do that. The coroutine form has three real advantages:
...
3. **Uniform NVTX tracing.** Every `Concern`-wrapped coroutine gets a `C<ClassName>.handle_batch` NVTX range for its full lifetime on the Driver stack -- nothing per-concern to plumb, nothing to forget. A plain method bypasses the stack and is invisible on the timeline.
