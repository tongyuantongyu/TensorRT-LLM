...
A single-phase concern that runs at the BATCH body's
``STATE_UPD_4``. Shaped as an ``async def handle_batch`` coroutine
(rather than a plain method) so the runtime gives it the same
automatic NVTX annotation as every other concern
(``CStateAdvanceConcern.handle_batch`` on the timeline) -- the
single ``await enter_phase`` rendezvous adds one line of
boilerplate but unlocks uniform tracing without per-concern
``nvtx.push_range`` plumbing.

Merges two legacy helpers that the original code kept apart only
because they were called from different sites
(``_update_request_states_tp`` and
...
What :meth:`handle_batch` does, by request bucket:
...
class StateAdvanceConcern:
    ...
    async def handle_batch(self, ctx: "Context") -> None:
        """Advance per-request state for the STATE_UPD_4 read view's batch.
        ...
        r, _ = await enter_phase(BatchPhase.STATE_UPD_4)
        if not r.can_queue:
            return
