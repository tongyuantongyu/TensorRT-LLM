...
class SampleConcern:
    ...
    async def handle_batch(self, ctx: "Context") -> None:
        ...
        # ran at STATE_UPD_4 in the BATCH body via
        # ``crn.state_advance.advance(ctx, scheduled_batch)`` -- it
        # has no data dependency on ``sample_state`` so it legitimately
        # belongs to its own concern at the earlier phase.
